...
void scheduler(void)
{
  ...
  for (;;)
  {
    struct proc *minproc = 0; // stores min proc ,i.e. proc made earliest in this case

    // Now find proc made earliest by traversing through proc list
    for (p = proc; p < &proc[NPROC]; p++)
    {
      intr_on();
      acquire(&p->lock);
      if (p->state == RUNNABLE)
      {
        if (!minproc || p->ctime < minproc->ctime)
        {
          if (minproc)
          {
    ...
    // We check if the minproc is RUNNABLE and not null and execute it
...
#endif
#ifdef PBS
  printf("PBS started lessgoo\n");
  for (;;)
  {
    intr_on();
    struct proc *pbs_proc = 0;
    int min_dp = 101; // stores and gets updated to min dp throughout the loop
    for (p = proc; p < &proc[NPROC]; p++)
    {
      acquire(&p->lock); // acquires lock to p
      ...
      if (p->state == RUNNABLE)
      {
        ...
        else if (pbs_proc && proc_dp < min_dp)
        {
          if (pbs_proc)
            release(&pbs_proc->lock);
          min_dp = proc_dp;
        ...
        else if (pbs_proc && proc_dp == min_dp && p->notime < pbs_proc->notime)
        {
          if (pbs_proc)
            release(&pbs_proc->lock);
          pbs_proc = p;
        ...
        else if (pbs_proc && proc_dp == min_dp && p->notime < pbs_proc->notime && p->ctime < pbs_proc->ctime)
        {
          if (pbs_proc)
            release(&pbs_proc->lock);
          pbs_proc = p;
    ...
    if (pbs_proc)
    {
      ...
      pbs_proc->start_time = ticks;
      pbs_proc->notime++;
      pbs_proc->run_time = 0;
      pbs_proc->sleep_time = 0;
      pbs_proc->wake_time = 0;
      c->proc = pbs_proc;
